...
// Naglagay tayo ng default fallback para hindi mag-crash kung walang maipasa ang parent
...
// FIXED: Nilagyan ng matinding safe checks para hindi mag-error kapag undefined ang data
const currentAnnouncements = computed(() => {
  if (!activeSubject.value) return []
  if (!props.announcementsData) return [] // Safe check 1
  
  // Safe check 2: Susubukan muna ang ID, tapos Code kung walang ID
